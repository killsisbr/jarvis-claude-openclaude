    ...
    async getPromptForCommand(args: string, context: ToolUseContext) {
      ...
      if (repo === null) {
        ...
        if (!hasAccess) {
          ...
          const msg = `Claude GitHub App not installed on ${repo.owner}/${repo.name} \u2014 install at https://claude.ai/code/onboarding?magic=github-app-setup if your trigger needs this repo.`
          setupNotes.push(msg)
        }
      }
